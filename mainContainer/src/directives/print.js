...
Print.prototype = {
  ...
  getStyle: function () {
    ...
    //   ? 'landscape'
    //   : 'portrait'
    // let str = `<style>@page{size: A4 portrait}</style>`
    let str = ''
    let styles = document.querySelectorAll('style,link')
    for (let i = 0; i < styles.length; i++) {
      str += styles[i].outerHTML
    }
    str += '<style>' + (this.options.noPrint ? this.options.noPrint : '.no-print') + '{display:none;} body{ color:#000 !important; }}</style>'

...
switch (process.env.NODE_ENV) {
  case 'development':
    socketUrl = 'ws://10.220.147.150:8091/ws'
    break
  case 'production':
    socketUrl = 'wss://ws.shuhaisc.com/ws'
    break
  case 'test':
    socketUrl = 'wss://testws.shuhaisc.com/ws'
    break
  case 'uat':
    socketUrl = 'wss://uatws.shuhaisc.com/ws'
    break
}
...
function openWebSocket () {
  ...
  try {
    accountId = (JSON.parse(read('userInfor')) || {}).accountId
  } catch (e) {
  }

  if (!accountId) return null
  let url = WEBSOCKET_URL + accountId
  try {
    ...
    if (this.list[ev]) {
      this.list[ev].forEach(v => {
        new Promise(() => {
          ...
          console.log('socket callback error:', err, v)
        })
      })
      return true
    } else {
...
export function connectSocket () {
  ...
  websocket.onmessage = msgEvent => {
    ...
    switch (data.messageStatus) {
      ...
      //   break
      case 2:
        socket.emit('updateUnreadNum', data)
        break
      default:
